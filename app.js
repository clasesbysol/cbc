...
async function loadSearch(){if(!sb)return;const [{data:s},{data:e}]=await Promise.all([sb.from('course_sections').select('unit_no,section_key,title,body_html'),sb.from('exercises').select('id,unit_no,title,statement_html')]);searchItems=[...(s||[]).map(x=>({label:`Unidad ${x.unit_no} · ${x.title}`,key:`section:${x.unit_no}:${x.section_key}`,text:`${x.title} ${x.body_html}`.replace(/<[^>]+>/g,' ')})),...(e||[]).map(x=>({label:`Unidad ${x.unit_no} · ${x.title}`,key:`exercise:${x.unit_no}:${x.id}`,text:`${x.title} ${formatStudyHtml(x.statement_html)}`.replace(/<[^>]+>/g,' ')}))]}
...
function unitVisual(s){const visualSets={1:{
  ...
  '1-del-atomo-indivisible-al-modelo-orbital':['atomic-models.svg','Evolución de los modelos atómicos desde Dalton hasta el modelo orbital'],
  '2-particulas-subatomicas-y-notacion-nuclear':['nuclear-notation.svg','Partículas subatómicas y lectura de la notación nuclear'],
  '5-bohr-niveles-cuantizados-y-luz':['bohr-spectrum.svg','Transiciones electrónicas del modelo de Bohr y emisión de luz'],
  '6-modelo-orbital-y-numeros-cuanticos':['orbitals.svg','Formas esquemáticas de orbitales s y p'],
  '7-configuracion-electronica':['electron-configuration.svg','Orden de llenado y distribución de electrones en orbitales'],
...
}};const v=visualSets[unit]?.[s.key];return v?`<figure class="chapterVisual"><img loading="lazy" src="./assets/chemistry/unit-${String(unit).padStart(2,'0')}/${v[0]}" alt="${v[1]}"><figcaption>${v[1]}</figcaption></figure>`:''}
function studySimulator(s){if(unit===1&&s.key==='cap-2')return `<section class="inlineSimulator" data-sim="states"><div class="simTitle"><span>SIMULADOR</span><h3>Estados de la materia</h3></div><div class="stateButtons"><button data-state="solid" class="active">Sólido</button><button data-state="liquid">Líquido</button><button data-state="gas">Gas</button></div><label>Temperatura relativa <input type="range" min="0" max="100" value="15" data-sim-input="temp"></label><svg class="particleStage" viewBox="0 0 520 230" role="img" aria-label="Partículas animadas del estado seleccionado"></svg><output data-sim-output></output></section>`;if(unit===1&&s.key==='cap-3')return `<section class="inlineSimulator" data-sim="density"><div class="simTitle"><span>SIMULADOR</span><h3>Laboratorio de densidad</h3></div><label>Masa (g) <input type="range" min="10" max="250" value="100" data-sim-input="mass"></label><label>Volumen (mL) <input type="range" min="10" max="200" value="80" data-sim-input="volume"></label><div class="densityScene"><div class="beaker"><span data-level></span></div><div class="scale">⚖ <b data-mass></b></div></div><output data-sim-output></output></section>`;if(unit===1&&s.key==='cap-6')return `<section class="inlineSimulator" data-sim="separation"><div class="simTitle"><span>SIMULADOR</span><h3>Elegí un método de separación</h3></div><label>Sistema <select data-sim-input="mixture"><option value="sand-water">Arena + agua</option><option value="oil-water">Aceite + agua</option><option value="salt-water">Sal disuelta + agua</option><option value="ink">Pigmentos de tinta</option></select></label><button class="primary small" data-check-separation>Comprobar método</button><output data-sim-output>Observá las propiedades de las fases y elegí el método.</output></section>`;if(unit===2&&s.key==='2-particulas-subatomicas-y-notacion-nuclear')return `<section class="inlineSimulator atomBuilder" data-sim="atom"><div class="simTitle"><span>SIMULADOR</span><h3>Constructor de átomos e iones</h3><p>Modificá el núcleo y la nube electrónica: símbolo, masa y carga cambian en tiempo real.</p></div><label>Protones, Z <input type="range" min="1" max="20" value="11" data-sim-input="p"></label><label>Neutrones <input type="range" min="0" max="24" value="12" data-sim-input="n"></label><label>Electrones <input type="range" min="0" max="22" value="11" data-sim-input="e"></label><div class="atomScene" aria-label="Modelo esquemático del átomo"><div class="atomNucleus" data-atom-nucleus></div><div class="atomOrbit"><i></i><i></i><i></i></div></div><output data-sim-output></output></section>`;if(unit===2&&s.key==='7-configuracion-electronica')return `<section class="inlineSimulator" data-sim="configuration"><div class="simTitle"><span>SIMULADOR</span><h3>Configuración electrónica hasta Ca</h3><p>Elegí el número atómico y observá cómo se llenan los subniveles.</p></div><label>Número atómico Z <input type="range" min="1" max="20" value="8" data-sim-input="z"></label><div class="electronBoxes" data-electron-boxes></div><output data-sim-output></output></section>`;if(unit===2&&s.key==='10-carga-nuclear-efectiva-y-propiedades-periodicas')return `<section class="inlineSimulator" data-sim="trends"><div class="simTitle"><span>SIMULADOR</span><h3>Comparador de tendencias periódicas</h3></div><label>Elemento A <select data-sim-input="a"><option>Li</option><option>Na</option><option>Mg</option><option>Al</option><option>Si</option><option>Cl</option><option>K</option></select></label><label>Elemento B <select data-sim-input="b"><option>Cl</option><option>Li</option><option>Na</option><option>Mg</option><option>Al</option><option>Si</option><option>K</option></select></label><output data-sim-output></output></section>`;return''}
...
const STUDY_CARDS={
  ...
  2:[
    ...
    ['¿Cómo cambia el radio atómico en un período?','En general disminuye de izquierda a derecha porque aumenta la carga nuclear efectiva.'],
    ['¿Hacia dónde aumenta la electronegatividad?','En general aumenta hacia la derecha y hacia arriba de la tabla periódica.']
  ]
};
...
function render(){if(!session&&!guest){app.className='';app.innerHTML=welcome();bind();return}let content=subjects();if(view==='chemistry')content=chemistry();if(view==='unit')content=unitPage();if(view==='math')content=math();if(view==='admin'&&isAdmin())content=admin()+adminSolutionsPanel();app.className='appShell';app.innerHTML=`${sidebar()}<div class="main">${renderTopbar()}<main class="content">${msg?`<div class="notice inline">${esc(msg)}</div>`:''}${content}</main></div>${searchOpen?searchModal():''}${themeOpen?themeModal():''}`;bind();renderMath();setTimeout(renderMath,250)}
function bind(){
