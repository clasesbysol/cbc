...
function exerciseCard(x){const sol=solutionsByExercise[x.id];return `<article id="exercise-${x.id}" class="exerciseCard"><div class="exerciseHead"><span>EJERCICIO</span><h3>${esc(x.title)}</h3></div><div class="exerciseStatement">${x.statement_html}</div>${sol?`<details class="solutionCard"><summary>${isAdmin()?'ADMIN · ✓ Resolución completa':'Ver resolución'}</summary><div class="solutionSteps">${sol.solution_html}${sol.final_answer?`<div class="finalAnswer"><b>Resultado</b>${sol.final_answer}</div>`:''}</div></details>`:''}</article>`}
...
function unitPage(){if(!canUnit(unit))return `<section class="lockedPage"><div class="bigIcon">🔒</div><span class="eyebrow">Unidad ${unit}</span><h1>Esta unidad no está habilitada.</h1><p>El acceso se asigna por unidad, capítulo o evaluación y puede vencer en una fecha determinada.</p></section>`;const sections=(sectionsByUnit[unit]||[]).filter(s=>canSection(unit,s));const exercises=exercisesByUnit[unit]||[];if(!sections.length&&!exercises.length)return `<section class="hero"><span class="eyebrow">Química · Unidad ${unit}</span><h1>${UNIT_TITLES[unit]||`Unidad ${unit}`}</h1><p>La estructura está lista; el contenido se incorporará más adelante.</p></section>`;return `<section class="unitHero"><span class="eyebrow">Química CBC · Unidad ${unit}</span><h1>${UNIT_TITLES[unit]||`Unidad ${unit}`}</h1><p>Teoría, ejemplos y práctica organizados para estudiar con claridad.</p><div class="unitStats"><span>${sections.length} secciones</span><span>${exercises.length} ejercicios</span></div></section><nav class="unitToc" aria-label="Índice de la unidad"><b>En esta unidad</b><div>${sections.map(s=>`<button data-scroll="${s.key}">${sectionLabel(s.type)} · ${esc(s.title.replace(/^\d+\.\s*/,''))}</button>`).join('')}${exercises.length?'<button data-scroll="exercises">EJERCICIOS</button>':''}</div></nav><div class="studySections">${sections.map((s,i)=>`<details id="sec-${s.key}" class="studySection ${s.type}" ${i===0?'open':''}><summary><div><small>${/simulador|calculadora/i.test(s.title)?'SIMULADOR':sectionLabel(s.type)}</small><h2>${esc(s.title)}</h2></div><span>⌄</span></summary><article>${s.html}${/simulador|calculadora/i.test(s.title)?simulatorWidget(s):''}</article></details>`).join('')}</div>${exercises.length?`<section id="sec-exercises" class="exerciseBook"><div class="sectionHeading"><span>PRÁCTICA</span><h2>Ejercicios</h2><p>Las resoluciones aparecen únicamente cuando están habilitadas para tu cuenta.</p></div>${exercises.map(exerciseCard).join('')}</section>`:''}`}
